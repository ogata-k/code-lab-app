...
sealed interface CommonDialogMessage {
    ...
    fun asString(): String {
        return when (this) {
            ...
            is RequestNavigateToNextGame -> stringResource(R.string.dialog_content_request_navigate_to_next_game).format(
                this.gridSize,
                this.gridSize,
                this.difficulty,
                this.estimateMaxScore
    ...
    data class RequestNavigateToNextGame(
        val gridSize: UInt,
        val difficulty: FifteenPuzzleDifficulty,
        val estimateMaxScore: UInt
    ) : CommonDialogMessage
...
sealed interface CommonDialogButtonText {
    ...
    fun asString(): String {
        return when (this) {
            ...
            Retry -> stringResource(R.string.btn_retry)
            ChangeSetting -> stringResource(R.string.btn_change_setting)
        }
    }
    ...
    data object Retry : CommonDialogButtonText
    data object ChangeSetting : CommonDialogButtonText
}
...
sealed interface CommonDialogData {
    ...
    data class ShowRequestActionDialog(
        ...
        val onCancel: ((showingDialog: ShowRequestActionDialog) -> Unit)? = null,
        val onDismiss: UiCallback = UiCallback({
        ...
        is CommonDialogData.ShowRequestActionDialog -> {
            BasicDialog(
                ...
                onDismissButtonRequest = {
                    onDismiss()
                    if (effect.onCancel != null) {
                        effect.onCancel.invoke(effect)
                    } else {
                        effect.onDismiss.invoke()
                    }
                },
